    ...
    map(~ {
      ...
      plot <- plot +
        ...
        annotate("text", x = min(df[[column_name]], na.rm = TRUE),
                 y = Inf, label = "DEFICIT",
                 hjust = 0, vjust = 1.5, color = "red", size = 3, fontface = "bold") +
        annotate("text", x = max(df[[column_name]], na.rm = TRUE),
                 y = Inf, label = "SURPLUS",
                 hjust = 1, vjust = 1.5, color = "blue", size = 3, fontface = "bold") +
        geom_rug(alpha = 0.2) +
        labs(
          title = paste("Distribution of", gsub("_", " ", column_name)) |>
            str_wrap(width = 20),
          x = paste(gsub("_", " ", column_name), "(Negative = Deficit, Positive = Surplus)") |>
            str_wrap(width = 40),
          y = "Density"
        ) 
