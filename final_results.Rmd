...
p = ggplot(baseline_data, aes(x = `Protein (g)_corrected`)) +
  geom_histogram(aes(y = after_stat(count)/sum(after_stat(count)) * 100),
                 fill = "#4A90E2", color = "black", binwidth = 2, size = 0.7, alpha = 0.8) +
  scale_y_continuous(labels = scales::percent_format(scale = 1)) +
  labs(title = "Distribution of Corrected Protein (g) Values",
       x = "Protein (g)",
       y = "Percentage") +
...
p = ggplot(baseline_data, aes(x = `Protein (g)_corrected`, fill = age_group)) +
  geom_histogram(aes(y = after_stat(count)/sum(after_stat(count)) * 100),
                 color = "black", binwidth = 2, position = "dodge", size = 0.7, alpha = 0.8) +
  scale_y_continuous(labels = scales::percent_format(scale = 1)) +
  scale_fill_brewer(palette = "Set2") +
  labs(title = "Distribution of Corrected Protein (g) Values by Age Group",
       x = "Protein (g)",
       y = "Percentage",
       fill = "Age Group") +
  theme_minimal(base_size = 14, base_family = "sans") +
...
p = ggplot(baseline_data_aggregated, aes(x = home_protein_consumption_new, fill = has_animal_protein)) +
  geom_histogram(aes(y = after_stat(count)/sum(after_stat(count)) * 100),
                 color = "black", position = "dodge", binwidth = 10, size = 0.7, alpha = 0.8) +
  scale_y_continuous(labels = scales::percent_format(scale = 1)) +
  scale_fill_brewer(palette = "Set1", labels = c("FALSE" = "No Animal Protein", "TRUE" = "Has Animal Protein")) +
  labs(title = "Distribution of Home Protein Consumption",
       x = "Home Protein Consumption (g)",
       y = "Percentage",
       fill = "Animal Protein") +
  theme_minimal(base_size = 14, base_family = "sans") +
  theme(
  ...
  output = columns |>
    map(~ {
      ...
      plot = ggplot(baseline_data_aggregated, aes(.data[[.x]])) +
        geom_histogram(aes(y = after_stat(count)/sum(after_stat(count)) * 100),
                       binwidth = by, fill = "#4A90E2", color = "black", size = 0.7, alpha = 0.8) +
        labs(title = paste("Distribution of", gsub("_", " ", .x)) |> str_wrap(width = 40),
             x = gsub("_", " ", .x),
             y = "Percentage") +
        scale_y_continuous(labels = scales::percent_format(scale = 1)) +
        theme_minimal(base_size = 14, base_family = "sans") +
      ...
      plot = ggplot(baseline_data_aggregated, aes(y = .data[[.x]], fill = age_group)) +
        geom_histogram(aes(x = after_stat(count) / sum(after_stat(count)) * 100,
                           text = paste("Percentage:", after_stat(count) / sum(after_stat(count)) * 100,
                                        "<br>", .x, after_stat(y))),
                       binwidth = by, position = "dodge", color = "black", size = 0.7, alpha = 0.8) +
...
satisfaction_dist_sex = function(columns, by = 20) {
  ...
  output = columns |>
    map(~ {
      # Create the distribution plot by sex
    ...
    map(~ {
      ...
      if (use_groups && fill_by_group) {
        ...
        plot <- ggplot(df, aes_string(x = column_name, fill = group_var)) +
          geom_density(alpha = 0.6, position = "identity", size = 0.8) +
          scale_fill_brewer(palette = "Set2") +
          theme_minimal(base_size = 14, base_family = "sans") +
          theme(
            plot.title = element_text(size = 16, face = "bold", hjust = 0.5, margin = margin(b = 10)),
        ...
        plot <- ggplot(df, aes(x = .data[[column_name]])) +
          geom_density(fill = "#4A90E2", alpha = 0.6, size = 0.8) +
          theme_minimal(base_size = 14, base_family = "sans") +
          theme(
            plot.title = element_text(size = 16, face = "bold", hjust = 0.5, margin = margin(b = 10)),
            axis.text.x = element_text(angle = 45, hjust = 1, size = 12, color = "black"),
      ...
      # Add common elements
      plot <- plot +
        geom_vline(xintercept = 0, color = "#D32F2F",
                   linetype = "dashed", linewidth = 1.2) +
        annotate("text", x = min(df[[column_name]], na.rm = TRUE),
                 y = Inf, label = "DEFICIT",
                 hjust = 0, vjust = 1.5, color = "#D32F2F", size = 5, fontface = "bold") +
        annotate("text", x = max(df[[column_name]], na.rm = TRUE),
                 y = Inf, label = "SURPLUS",
                 hjust = 1, vjust = 1.5, color = "#1976D2", size = 5, fontface = "bold") +
        geom_rug(alpha = 0.3, size = 0.5) +
        labs(
          title = paste("Distribution of", gsub("_", " ", column_name)) |>
            str_wrap(width = 30),
          x = paste(gsub("_", " ", column_name), "\n(Negative = Deficit, Positive = Surplus)") |>
            str_wrap(width = 50),
          y = "Density"
        ) 
  ...
  column_names |>
    map(
      ~{
        ...
        median_col = paste0(.x, "_median")
        result = result |>
          mutate(Interpretation = ifelse(get(median_col) < 0, "DEFICIT", "SURPLUS"))
        ...
        result |>
          datatable(options = list(pageLength = 15,
                                   ...
                                   dom = 'Bfrtip'), filter = 'top',
                    extensions = 'Buttons',
                    caption = paste("Distribution of ", .x, "(Negative = Deficit, Positive = Surplus)"))
      }
    )
}
      ...
      ~{
        ...
        comparisons |>
          ...
          datatable(options = list(pageLength = 15,
                                   ...
                                   dom = 'Bfrtip'), filter = 'top',
                    extensions = 'Buttons',
                    caption = paste("Distribution of ", .x, " by Age group (Negative = Deficit, Positive = Surplus)"))
      }
    )
}
      ...
      ~{
        ...
        comparisons |>
          ...
          datatable(options = list(pageLength = 25,
                                   ...
                                   dom = 'Bfrtip'), filter = 'top',
                    extensions = 'Buttons',
                    caption = paste("Distribution of ", .x, " by Child Sex (Negative = Deficit, Positive = Surplus)"))
      }
    )
}
      ...
      ~{
        ...
        comparisons |>
          ...
          datatable(options = list(pageLength = 25,
                                   ...
                                   dom = 'Bfrtip'), filter = 'top',
                    extensions = 'Buttons',
                    caption = paste("Distribution of ", .x, " by Age group and Child Sex (Negative = Deficit, Positive = Surplus)"))
      }
    )
}
```
