...
baseline_data = baseline_data |>
  mutate(
    ...
    animal_protein_presence = ifelse(stringr::str_detect(ingredient_name, "milk|beef|eggs|fish"), "animal based", "plant based"), # assign protein type i.e milk|beef|eggs|fish as animal and the rest as plant
  )  |>
...
baseline_data = baseline_data |>
  mutate(
    ...
    animal_protein_presence = ifelse(stringr::str_detect(ingredient_name_simplified, "milk|beef|eggs|fish"), "animal based", "plant based"), # assign protein type i.e milk|beef|eggs|fish as animal and the rest as plant
...
baseline_data = baseline_data |>
  mutate(
    volume_calc = ifelse(str_detect(ingredient_name_simplified, "tilapia"), volume_calc * 0.33, volume_calc),
    volume_consumed = (volume_calc * 100))
```
...
volumes = baseline_data |>
  ...
  summarise(
    total_food_volume = sum(volume_calc * 100)
  ) |> ungroup() |> distinct()
```
...
outlier_points |>
  left_join(
    baseline_data |>
      select(participant_id, time, ingredient_name, volume_calc) |>
      distinct()) |>
  relocate(ingredient_name, time, volume_calc, .after = food_item_proc_std)  |>
  arrange(participant_id, food_item_proc_std, time, ingredient_name, volume_calc) |>
...
royco = baseline_data |>
  filter(str_detect(ingredient_name_simplified, "royco") & volume_consumed > 5)

royco |>
  select(participant_id, time, food_item_proc_std, ingredient_name, volume_calc, volume_consumed) |>
  mutate(across(where(is.numeric), ~ round(.x, 2))) |>
  datatable(options = list(pageLength = 15, 
                           scrollX = TRUE,
                           dom = 'Bfrtip'), filter = 'top',  
...
coriander = baseline_data |>
  filter(ingredient_name_simplified == "dhania" & volume_consumed > 30)
...
15. *ingredient_name_simplified*: created from *ingredient_name* to facilitate better matching with protein digestibility and AA content datasets (*NOTE:* an intermediary column not used in matching)
16. *ingredient_name_simplified_digestibility*: created from *ingredient_name_simplified* optimizing it for matching with protein digestibility
17. *ingredient_name_simplified_AA*: created from *ingredient_name_simplified* optimizing it for matching with AA content
18. *Protein_Source_simplified*: extract from the *Protein_Source* column in protein digestibility data for simplicity matching with *ingredient_name_simplified_digestibility*
19. *food_name_simplified*: extract from the *Food name in English* amino acid contents data for simplicity matching with *ingredient_name_simplified_AA*
